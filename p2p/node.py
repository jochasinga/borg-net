...
class Node:
    ...
    def __init__(self, maxpeers, serverport, myid=None, serverhost=None):
        ...
        information for up to maxpeers number of peers (set to 0 for unlimited
        number of peers), listening on a given server port, with a given canonical
        peer name (id) and host address. If not supplied, the host address
        (serverhost) will be determine by attempting to connect to an Internet
        host like Google.
        """
        self.debug = False
        ...
        # Ensure proper access to peers list
        # (maybe better to use threading.RLock (reentrant))
        self.peerlock = threading.Lock()
        self.peers = {}             # peerid ==> (host, port) mapping
        self.shutdown = False       # used to stop the main loop

        # Dictionary mapping handlers  {msgtype : handler}
        self.handlers = {}
        self.router = None
    ...
    def __handlepeer(self, clientsock):
        ...
        try:
            ...
            # Uppercase the msgtype -> i.e. "PING"
            if msgtype:
                msgtype = msgtype.upper()
            # This means handlers must be named after msgtypes
            if msgtype not in self.handlers:
                self.__debug('Not handled: {0}: {1}'.format(msgtype, msgdata))
            else:
                self.__debug('Handling peer msg: {0}:{1}'.format(msgtype, msgdata))
                # call the handler and pass the connection and msgdata
    ...
    def addrouter(self, router):
        ...
        is as follows:
            ...
            - The routing function should take the name of a peer (which may not
              necessarily be present in self.peers) and decide which of the known
              peers a message should be routed to next in order to (hopefully)
              reach the desired peer.
    ...
    def addpeer(self, peerid, host, port):
        """
        Add a peer name nad host:port mapping to the known list of peers.
        """
    ...
    def removepeer(self, peerid):
        """Removes peers information from the known list of peers."""
    ...
    def maxpeersreached(self):
        ...
        assert self.maxpeers == 0 or len(self.peers) <= maxpeers
    ...
    def makeserversocket(self, port, backlog=5):
        ...
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        # SO_REUSEADDR make the port reusable right away after server shutdown
    ...
    def sendtopeer(self, peerid, msgtype, msgdata, waitreply=True):
        ...
        If no router function has been registered, it won't work. The router
        function should provide the next immediate peer to whom the message
        should be forwarded. The peer's reply, if it is expected,
        will be returned.

        Returns None if the message could not be routed.
        """
        if self.router:
            next.pid, host, port = self.router(peerid)
        if not self.router or not nextpid:
            self.__debug('Unable to route {0} to {1}'.format(msgtype, peerid))
            return None
    ...
    def connectandsend(self, host, port, msgtype, msgdata, pid=None, waitreply=True):
        ...
        try:
            ...
            self.__debug('Sent {0}:{1}'.format(pid, msgtype))

            if waitreply:
                onereply = peerconn.recvdata()
                while (onereply != (None, None)):
                    msgreply.append(onereply)
                    self.__debug('Got reply {0}: {1}'.format(
                        pid, str(msgreply)))
                    onereply = peerconn.recvdata()
                peerconn.close()
        except KeyboardInterrupt:
    ...
    def mainloop(self):
        ...
        s.settimeout(2)
        self.__debug('Server started: {0} ({1}:{2})'.format(
...
class NodeConnection:
    ...
    def senddata(self, msgtype, msgdata):
    	...
    	"""
        try:
            msg = self.__makemsg(msgtpe, msgdata)
            self.sd.write(msg)
            self.sd.flush()
        except KeyboardInterrupt:
            raise
    ...
    def recvdata(self):
        ...
        try:
            ...
            while len(msg) != msglen:
                data = self.sd.read(ming(2048, msglen - len(msg)))
                if not len(data):
                     break
                msg += data
    ...
    def __str__(self):
        return "|{peerid}|".format(peerid=peerid)
